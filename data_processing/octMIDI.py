...
def encoding_to_str(e, bar_index_offset=0):
    ...
    return ' '.join((['<s>'] * tokens_per_note)
                    + ['<{}-{}>'.format(j, k if j > 0 else k + bar_index_offset) for i in e if i[0] + bar_index_offset < bar_max for j, k in enumerate(i)]
                    + (['</s>'] * (tokens_per_note)))
...
if __name__ == '__main__':
    midi_dir = "/data1/zhaojw/PIAST/PIAST/piast_yt/midi/-cLYVsf1R1E.mid"
    midi_obj = miditoolkit.midi.parser.MidiFile(midi_dir)
    encoding = MIDI_to_encoding(midi_obj)
    print(encoding[:100])
    #midi = encoding_to_MIDI(encoding)
    #midi.dump("test_recon.mid")