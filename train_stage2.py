...
from dataset import Ensemble_Dataset, collate_fn_musecoco2
...
def main(rank, world_size, log_path_mng, VERBOSE, MODEL_NAME):
    ...
    model = Blip2Musecoco(load_pretrained=False, stage_1_checkpoint='/data2/zhaojw/blip2_style_learning_WO_LMD_224notes_12bins_009_epoch.pt')#'/data2/zhaojw/blip2_style_learning_WO_LMD_224notes_12bins_009_epoch.pt'
    ...
    train_loader = DataLoader(train_set, batch_size=BATCH_SIZE, drop_last=True, shuffle=False, collate_fn=lambda b: collate_fn_musecoco2(b, rank, augment=True, deperf=True), sampler=DistributedSampler(train_set)) 

    val_set = Ensemble_Dataset(debug_mode=VERBOSE, split='validation', load_num=30, pitch_order='high_to_low')
    val_loader = DataLoader(val_set, batch_size=BATCH_SIZE, drop_last=True, shuffle=False, collate_fn=lambda b: collate_fn_musecoco2(b, rank, augment=True, deperf=True), sampler=DistributedSampler(val_set)) 
    ...
    scheduler = CosineAnnealingLR(optimizer, T_max=len(train_loader)*N_EPOCH-WARMUP_STEP, eta_min=LR*0.1)
...
if __name__ == '__main__':
    ...
    MODEL_NAME = 'revamped_blip2_a2s_musecoco_PIAST_POP909_LoRA'
    DEBUG = 0
    ...
    if DEBUG:
        ...
        save_root = '/data2/zhaojw/blip2_a2s/'
        log_path_name = MODEL_NAME


    readme_fn = 'train_stage2.py'
